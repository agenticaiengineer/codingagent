...
|-------|------|-------------|
| Layer 6 | **Multi-Agent Orchestration** | Coordinator + specialized workers, parallelism |
| Layer 5 | **Agentic Loop (ReAct)** | `while(not_done): think → act → observe → repeat` |
| Layer 4 | **Tool Use / Function Calling** | File I/O, terminal, search, browser, APIs |
...
2. Tokenizer
3. Embeddings
4. Transformer (N layers of self-attention)
5. Prediction Head (linear + softmax)
6. Next Token
7. Append
8. Repeat
...
|------|-------|-------------|
| 1 | Define Tools | Provide function schemas and parameter definitions |
| 2 | Model Decides | Model chooses direct response or tool call(s) |
| 3 | Execute Function | Application invokes the requested function |
| 4 | Return Results | Tool output is sent back to the model |
| 5 | Final Response | Model returns final response (or next tool call) |
...
|------|---------|--------|-------------|
| 1 | I need to understand the current code before making changes. | `read_file("src/main.py")` | `[file contents...]` |
| 2 | The bug is on line 42; comparison should be `>=` not `==`. | `edit_file("src/main.py", line=42, old="==", new=">=")` | File edited successfully. |
| 3 | Now run tests to verify the fix. | `run_command("pytest tests/ -v")` | `15 passed, 0 failed.` |
| 4 | All tests pass. The fix is correct. | Final response to user | — |
...
1. User input is combined with system prompt and tool definitions.
2. Claude API returns either tool calls or a final response.
3. If tool calls are returned, execute tools, append results, and loop.
4. If no tool calls are returned, emit final text response to the user.
...
1. User request is sent to an Architect model (for high-level solution planning).
2. Architect plan is passed to an Editor model (for precise repository edits).
3. Resulting changes are committed to git.
...
|-------|-------|----------------|
| 1 | Lead agent (orchestrator, e.g., Claude Opus) | Decomposes task and spawns workers |
| 2 | Sub-agent 1 (e.g., Sonnet) | Research |
| 3 | Sub-agent 2 (e.g., Sonnet) | Code implementation |
| 4 | Sub-agent N (e.g., Sonnet) | Testing / validation |
| 5 | Lead agent (synthesizer) | Merges results into a coherent output |
...
3. **Test**
4. **Observe**
5. If errors are found, **Debug** and loop back to **Edit**/**Test**.
